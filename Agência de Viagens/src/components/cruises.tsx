import { useState, useEffect } from "react";
import { Card, CardContent } from "./ui/card";
import { Badge } from "./ui/badge";
import { Button } from "./ui/button";
import { Calendar, Users, Star, Loader2 } from "lucide-react";
import { ImageWithFallback } from "./figma/ImageWithFallback";
import { cruisesAPI } from "../utils/api";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogFooter,
} from "./ui/dialog";





export function Cruises() {
  const [cruises, setCruises] = useState<any[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [selectedCruise, setSelectedCruise] = useState<any | null>(null);
  const [isDetailsOpen, setIsDetailsOpen] = useState(false);


  useEffect(() => {
    loadPromotions();
  }, []);

  const abrirWhatsapp = (titulo: string) => {
    const phoneNumber = "5584987432387";
    const message = "Olá, gostaria de mais informações sobre as promoções de viagens. " + titulo;
    const url = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`;
    window.open(url, "_blank");
  };


    const openDetails = (promo: any) => {
      setSelectedCruise(promo);
      setIsDetailsOpen(true);
    };

  const loadPromotions = async () => {
    try {
      setIsLoading(true);
      const data = await cruisesAPI.getAll();
      setCruises(data.cruises || []);
    } catch (error) {
      console.error("Error loading cruises:", error);
    } finally {
      setIsLoading(false);
    }




  };
  return (
    <section id= "cruises" className = "py-20" >
      <div className="container mx-auto px-4" >
        <div className="text-center mb-12" >
          <h2 className="text-4xl mb-4" > Cruzeiros </h2>
            < p className = "text-xl text-gray-600 max-w-2xl mx-auto" >
              Aproveite nossas ofertas exclusivas e realize a viagem dos seus sonhos
                </p>
                </div>

  {
    isLoading ? (
      <div className= "flex items-center justify-center h-64" >
      <Loader2 className="w-8 h-8 animate-spin text-blue-600" />
        </div>
        ) : cruises.length === 0 ? (
      <div className= "text-center text-gray-500 py-12" >
      <p>Nenhum cruzeiro disponível no momento.</p>
        < p className = "text-sm mt-2" > Volte em breve para conferir nossas ofertas! </p>
          </div>
        ) : (
      <div className= "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" >
      {
        cruises.map((cruise, index) => (
          <Card key= { index } className = "overflow-hidden hover:shadow-xl transition group" >
          <div className="relative h-64 overflow-hidden" >
        <ImageWithFallback
                  src={ cruise.image }
                  alt = { cruise.name }
                  className = "w-full h-full object-cover group-hover:scale-110 transition duration-300"
          />
          <Badge className="absolute top-4 right-4 bg-red-600 text-white" >
          { cruise.discount }
          </Badge>
          </div>
        < CardContent className = "p-6" >
        <div className="flex items-center gap-2 mb-2" >
        <span className="text-sm text-blue-600" > { cruise.name } </span>
        < div className = "flex items-center gap-1 ml-auto" >
        <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
        <span className="text-sm" > { cruise.rating } </span>
        </div>
        </div>
        < h3 className = "text-xl mb-3" > { cruise.title } </h3>
        < div className = "flex items-center gap-4 text-sm text-gray-600 mb-4" >
        <div className="flex items-center gap-1" >
        <Calendar className="w-4 h-4" />
        </div>
        < div className = "flex items-center gap-1" >
        <Users className="w-4 h-4" />
        </div>
        </div>
        < div className = "flex items-end justify-between mb-4" >
        <div>
        <p className="text-sm text-gray-500 line-through" > { cruise.price } </p>
        < p className = "text-2xl text-blue-600" > { cruise.price } </p>
        </div>
        </div>
        < Button className = "w-full" onClick = {() => openDetails(cruise)} >
      Ver Detalhes
        </Button>
        </CardContent>
        </Card>
        ))
  }
  </div>
        )
}
</div>
<Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
  <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
    {selectedCruise && (
      <>
        <DialogHeader>
          <DialogTitle>{selectedCruise.name}</DialogTitle>
        </DialogHeader>

        <div className="space-y-4">
          <ImageWithFallback
            src={selectedCruise.image}
            alt={selectedCruise.name}
            className="w-full h-64 object-cover rounded-lg"
          />

          <div className="flex items-center justify-between">
            <Badge className="bg-red-600 text-white">
              {selectedCruise.price}
            </Badge>

            <div className="flex items-center gap-1">
              <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
              <span>{selectedCruise.rating}</span>
            </div>
          </div>

          <p className="text-gray-600">
            📍 <strong>Destino:</strong> {selectedCruise.destination}
          </p>

          <div className="flex gap-6 text-sm text-gray-600">
            <div className="flex items-center gap-1">
              <Calendar className="w-4 h-4" />
              {selectedCruise.price}
            </div>
            <div className="flex items-center gap-1">
              <Users className="w-4 h-4" />
              {selectedCruise.price}
            </div>
          </div>

          <div className="border-t pt-4">
            <p className="text-sm text-gray-500 line-through">
              {selectedCruise.price}
            </p>
            <p className="text-3xl text-blue-600 font-bold">
              {selectedCruise.price}
            </p>
          </div>

          {selectedCruise.description && (
            <p className="text-gray-700 leading-relaxed">
              {selectedCruise.description}
            </p>
          )}
        </div>

        <DialogFooter>
          <Button variant="outline" onClick={() => setIsDetailsOpen(false)}>
            Fechar
          </Button>
          <Button onClick={() => abrirWhatsapp(selectedCruise.name)}>Solicitar reserva</Button>
        </DialogFooter>
      </>
    )}
  </DialogContent>
</Dialog>

  </section>
  
  );

}
